...
import { FerramentasContainer, Mensagem, Badge, Icone } from "./styles";
import LoginButton from "../LoginButton/LoginButton";
import UsuarioMenu from "../UsuarioMenu/UsuarioMenu";
import Notificacao from "../Notificacao/Notificacao";

const Ferramentas = ({ usuario, handleLogout }) => {
  const [menuUsuarioAberto, setMenuUsuarioAberto] = useState(false);
  const [menuNotificacaoAberto, setMenuNotificacaoAberto] = useState(false);
  const ferramentasRef = useRef(null);

  // Fecha ambos os menus se clicar fora
  useEffect(() => {
    const handleClickFora = (e) => {
      if (ferramentasRef.current && !ferramentasRef.current.contains(e.target)) {
    ...
    <FerramentasContainer ref={ferramentasRef}>
      ...
      <Mensagem>
        ...
        <Icone xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
          ...
          0 7-2.807 7-6s-3.004-6-7-6-7 2.808-7 
          6c0 1.468.617 2.83 1.678 3.894m-.493 
          3.905a22 22 0 0 1-.713.129c-.2.032-.352-.176-.273-.362a10 
          10 0 0 0 .244-.637l.003-.01c.248-.72.45-1.548.524-2.319C.743 
          11.37 0 9.76 0 8c0-3.866 3.582-7 8-7s8 
          3.134 8 7-3.582 7-8 7a9 9 0 0 
      ...
      <Notificacao
        usuario={usuario}
        menuAberto={menuNotificacaoAberto}
        setMenuAberto={setMenuNotificacaoAberto}
        fecharMenuUsuario={() => setMenuUsuarioAberto(false)}
      />
      ...
      {usuario ? (
        <UsuarioMenu
          ...
          menuAberto={menuUsuarioAberto}
          setMenuAberto={setMenuUsuarioAberto}
          fecharMenuNotificacao={() => setMenuNotificacaoAberto(false)}
        />
      ) : (
        <LoginButton />
      )}
